...
def get_offset_check(tim,x_cen,y_cen):
        ...
        out = np.where(coimg==coimg.max())
        print('pixels which are at maximum')
        print(out)
        yloc_out = out[0].mean()
        xloc_out = out[1].mean()
        flag, xx, yy = targetwcs.radec2pixelxy(*(tim.subwcs.pixelxy2radec(float(x_cen+1), float(y_cen+1))[-2:]))
...
def get_center_pix(tim,target_x,target_y):
     #Jun9 input x,y coordinat, output x,y on coimg
        global BRICKNAME
        global SURVEY_DIR
        survey = LegacySurveyData(survey_dir=SURVEY_DIR)
        ...
        imgs = [patched]
        Yo,Xo,Yi,Xi,rimgs = resample_with_wcs(targetwcs, tim.subwcs, imgs, 1, intType=np.int16)
        idx = np.where((Yo==target_y)&(Xo==target_x))
        if len(idx)>0:
            try:
              idx=idx[0][0]
              import pdb;pdb.set_trace()
              print(Yi[idx],Xi[idx])
              print(np.where((Yi==Yi[idx])&(Xi==Xi[idx])))
              idxes=np.where((Yi==Yi[idx])&(Xi==Xi[idx]))[0]
              for idx in idxes:
                  print(Yo[idx],Xo[idx])
...
class BuildStamp():
      ...
      def setlocal(self,obj):#get a sub_tim image as what's been done in fitblobs stage. see _blob_iter? code for details
              ...
              self.target_x=x
              self.target_y=y
              #xx1,yy1 = get_center_pix(self.tim,self.target_x,self.target_y)
      ...
      def elg(self,obj):
          ...
          if mod0.sum()>0:
              idx=np.where(mod0==mod0.max())
              y=idx[0].mean()
              x=idx[1].mean()
              offset_y = 33-y
              offset_x = 33-x
              if np.abs(offset_x)>3 or np.abs(offset_y)>3:
                  offset_x=0
                  offset_y=0
          else:
              offset_y = 0
          ...
          galsim_img.bounds.xmin=self.sx0+int(offset_x)
          galsim_img.bounds.xmax=self.sx1-1
          galsim_img.bounds.ymin=self.sy0+int(offset_y)
...
class BuildStamp_old():
    ...
    def setlocal(self,obj):
        ...
        flag, xx, yy = self.tim.subwcs.radec2pixelxy(*(self.targetwcs.pixelxy2radec(x+1,y+1)[-2:]))
        if xx1 is not None:
            if (xx1!=int(xx-0.5))|(yy1!=int(yy-0.5)):
                import pdb;pdb.set_trace()
                xx1,yy1 = get_center_pix(self.tim,self.target_x,self.target_y)
                flag, xx, yy = self.tim.subwcs.radec2pixelxy(*(self.targetwcs.pixelxy2radec(x+1,y+1)[-2:]))
    ...
    def elg(self,obj):
        ...
        import pdb;pdb.set_trace()
        gal.setCenter(self.xpos+1, self.ypos+2)
        idx = np.where(gal.array==gal.array.max())
        y=idx[0]
        x=idx[1]
        offset_y=12-y
        offset_x=12-x
        gal.setCenter(self.xpos+1+offset_x,self.ypos+1+offset_y)
        return gal
